...
import { getPveAddress } from "@/api/exam";

let questionAnswerType = (item, answer = false, setUpChoose = null, shouldInput = false, role, previewType, examCategory) => {
  ...
  if (item.type === 2 || item.type === 1) {
    // 如果是学生正在答题的时候
    if (previewType === "answer" || previewType === "view") {
      return (
        <ul className="question-options">
          {options.map((val, index) => {
            return (
              item[`option${val}`] && (
                <li className="options_item" key={index} onClick={() => (setUpChoose ? setUpChoose(item.type, item.id, val) : null)}>
                  {item.type === 1 ? (
    ...
    const studentAnswer = item.examPaperAnswerVO?.answer ?? ""; // 学生答题完,考试提交的答案
    return (
      <ul className="question-options">
        {options.map((val, index) => {
          return (
            item[`option${val}`] && (
              <li className="options_item" key={index} onClick={() => (setUpChoose ? setUpChoose(item.type, item.id, val) : null)}>
                {item.type === 1 ? (
                  <Radio checked={studentAnswer ? studentAnswer.indexOf(val) !== -1 : false} />
                ) : (
                  <Checkbox checked={studentAnswer ? studentAnswer.indexOf(val) !== -1 : false} />
                )}
                <span className="label">{val}.</span>
                {item[`option${val}`]}
              </li>
    ...
    const targetValue = previewType === "answer" ? answer : item.examPaperAnswerVO?.answer ?? "";

    return (
      <div className="question-options">
        <Button onClick={() => setUpChoose(item.type, item.id, "Y")} type={targetValue === "Y" ? "primary" : "default"}>
          正确
        </Button>
        <Button
          style={{ marginLeft: 20 }}
          onClick={() => (setUpChoose ? setUpChoose(item.type, item.id, "N") : null)}
          type={targetValue === "N" ? "primary" : "default"}
        >
          错误
        </Button>
      </div>
    );
  } else if (item.type === 4 && shouldInput && !["T", "TM"].includes(role)) {
    return (
      <Input.TextArea
        value={item.answer}
        placeholder="请输入你的答案"
          ...
          <div className={"question-annex-fileList"}>
            ...
            {imgs.length ? (
              <div className={"ans-imgs"}>
                {/* <PhotoProvider imageClassName={"ans-img"} viewClassName={"ans-imgs-wrap"} toolbarRender={handleToolbarRender}>
                  {imgs.map((item, index) => (
            ...
            <div className={"ans-others"}>
              {others.map((item) => (
                <a style={{ display: "block" }} key={item.id} onClick={() => download(`/newexam/examNewResult/downLoadAnswerFile/${item.id}`)}>
                  {item.fileName}
                </a>
              ))}
            </div>
    ...
    if (role === "S") {
      if (shouldInput) {
        return (
          <div style={style}>
            {/*当学生答题时,并且是1+x 考试,并且是技能题时,才出现这个按钮  examCategory 就是考试类型*/}
            {examCategory === 2 && (
...
const Question: React.FC<{
   ...
   **/
  const { setUpChoose, role = "T", shouldInput = false, previewType, isMob, downloadUrlState, examCategory } = _props;
  const {
    category,
    majorName,
    lessionName,
  ...
  return (
    <div id="com-question-wrapper">
      <div className="com-question" style={{ marginTop: "5px" }}>
        ...
        {FILELIST && FILELIST.length > 0 && (
          <div className="question-annex-wrapper">
            ...
            <div className="question-annex-fileList">
              {FILELIST.map((item) => {
                return (
                  <a style={{ display: "block" }} key={item.id} onClick={() => download(`${downloadUrl}${item.id}${formatDownloadArgs(downloadUrlState)}`)}>
                    {item.fileName}
                  </a>
                );
              })}
        ...
        {(() => {
          console.log(role, shouldInput);
        })()}
        {questionAnswerType(props, answer, role === "S" && shouldInput ? setUpChoose : null, shouldInput, role, previewType, examCategory)}
        {/*答案解析除答题外时间都显示*/}
        {(role === "T" || (role === "S" && !shouldInput) || role === "PM" || role === "M" || role === "TM") && (
          <div>
            ...
            {props.examPaperAnswerVO && utils.isValid(props.examPaperAnswerVO.teacherComment) && previewType !== "view" && (
